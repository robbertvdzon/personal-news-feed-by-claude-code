...
- [ ] Fetcht de code zelf een URL? Dan draait `SsrfUrlValidator.validate(...)` vlak vóór die fetch én **vóór** het opbouwen van het `HttpRequest`. `HttpRequest.Builder.uri(...)` weigert niet-`http`/`https` namelijk zelf al met een `IllegalArgumentException`; staat de bouwregel eerder, dan wint de JDK van je eigen check en komt er `invalid URI scheme file` in `external_calls` te staan in plaats van `geblokkeerd: alleen http/https-URLs zijn toegestaan` (SF-2249). De fetch faalt in beide gevallen — alleen is de afwijzing in het eerste geval niet meer als de onze herkenbaar. De bijbehorende test asserteert daarom op `errorMessage` bevat `"geblokkeerd"`, niet alleen op "de fetch leverde niets op".
