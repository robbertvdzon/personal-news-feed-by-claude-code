...
- Spring Modulith-moduleregels worden afgedwongen door `ModuleStructureTest` (`ApplicationModules…verify()`, lege allowlist); die test bestaat al en draait bij elke `mvn test` (zie `specs/backend-technical-spec.md` §7 en `development.md`). Raadpleeg/los die test op bij nieuwe modulegrens-schendingen in plaats van een nieuwe test toe te voegen; verifieer overige wijzigingen met `mvn test`.
