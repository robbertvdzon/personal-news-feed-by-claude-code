...
|---|---:|---:|---:|---|
| Productie (standaard) | 384 MiB | 640 MiB | 320 MiB | `deploy/base/backend-memory.yaml` |
| Preview | 320 MiB | 448 MiB | 192 MiB | `deploy/overlays/preview/backend-memory.yaml` |
...
De ApplicationSet in `robberts-infrastructure` past hetzelfde previewprofiel als laatste toe. Dit dekt ook featurebranches die deze bestanden nog niet hebben. Pas bij een ander previewbudget zowel de overlay als de centrale patch aan. Dit is een startbudget; zware features kunnen meer nodig hebben.
