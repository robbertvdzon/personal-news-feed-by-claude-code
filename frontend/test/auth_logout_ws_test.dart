...
/// Praat niet met het netwerk: `/api/requests` levert een lege lijst.
...
void main() {
  ...
  test('na logout en login van een andere gebruiker verbindt de socket met het nieuwe token', () async {
    ...
    await container.read(authProvider.notifier).storeSession('token-van-a', 'a', 'user');
    ...
    await container.read(authProvider.notifier).storeSession('token-van-b', 'b', 'user');
