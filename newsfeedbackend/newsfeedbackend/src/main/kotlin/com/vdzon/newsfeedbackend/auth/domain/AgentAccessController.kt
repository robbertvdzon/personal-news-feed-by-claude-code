...
@RestController
class AgentAccessController(private val access: AgentAccessVerifier, private val users: com.vdzon.newsfeedbackend.auth.infrastructure.UserRepository, private val jwt: com.vdzon.newsfeedbackend.auth.infrastructure.JwtService) {
    ...
    fun login(@RequestHeader("X-AI-Access-Token", required = false) token: String?,
        ...
        val user = users.findByUsername(email) ?: throw org.springframework.web.server.ResponseStatusException(org.springframework.http.HttpStatus.UNAUTHORIZED)
        return mapOf("token" to jwt.create(user.username, user.role, 3600), "email" to user.username, "role" to user.role)
    }
...
<script>document.getElementById('login').onsubmit=async(event)=>{event.preventDefault();const field=document.getElementById('token');const status=document.getElementById('status');try{const result=await fetch('/api/auth/agent-session',{method:'POST',headers:{'Content-Type':'application/json','X-AI-Access-Token':field.value},body:JSON.stringify({email:document.getElementById('email').value})});field.value='';if(!result.ok)throw new Error('Aanmelden geweigerd ('+result.status+').');const data=await result.json();localStorage.setItem('flutter.token', JSON.stringify(data.token));
localStorage.setItem('flutter.username', JSON.stringify(data.email));
localStorage.setItem('flutter.role', JSON.stringify(data.role)); status.textContent='Je werkt nu als '+data.email;document.getElementById('open').hidden=false;}catch(error){field.value='';status.textContent=error.message;}};</script></body></html>"""
    }
}
