...
class PostgresFeedItemRepository(
    ...
    override fun load(username: String): MutableList<FeedItem> =
        jdbc.query(
    ...
    override fun save(username: String, items: List<FeedItem>) {
        // Volledige replace, gelijk aan JSON-impl (writeJson overschrijft de hele file).
        jdbc.update(
            "DELETE FROM feed_items WHERE username = :u",
            MapSqlParameterSource("u", username)
        )
        items.forEach { upsert(username, it) }
    }

    override fun upsert(username: String, item: FeedItem): FeedItem {
        jdbc.update(
            """
