...
class PodcastServiceImpl(
    ...
    override fun audioBytes(username: String, id: String): ByteArray? {
        val podcast = repo.load(username).find { it.id == id }
        return if (podcast?.audioData != null) {
            log.info("[Podcast] audio read id={} user='{}' bytes={}", id, username, podcast.audioData.size)
            podcast.audioData
        } else {
            log.warn("[Podcast] audio missing id={} user='{}'", id, username)
            null
        }
    }
}
