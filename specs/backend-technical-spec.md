...
Een security-bump van een door de parent gemanagede component is daarom **één regelwijziging**: verhoog de parent-versie en voeg géén nieuwe expliciete `<version>`-pin toe. Zo bracht SF-2116 4.0.6 → 4.0.7 (o.a. `tomcat-embed-core` 11.0.21 → 11.0.22, `spring-websocket` 7.0.7 → 7.0.8, `micrometer-core` 1.16.5 → 1.16.6) en SF-2270 4.0.7 → 4.0.8, waarmee de vier artefacten uit zes OSV-advisories in één keer meeschoven: `org.apache.logging.log4j:log4j-api` 2.25.4 → 2.25.5, `org.postgresql:postgresql` 42.7.11 → 42.7.13, `com.fasterxml.jackson.core:jackson-databind` 2.21.4 → 2.21.5 en `tools.jackson.core:jackson-databind` 3.1.4 → 3.1.5. Verifieer de opbrengst met `mvn dependency:tree` (of `unzip -l target/newsfeedbackend-1.0.0.jar` op `BOOT-INF/lib`) in plaats van met de verwachte versietabel uit de story: één patch-bump verschuift tientallen beheerde versies tegelijk (SF-2270: 39), dus de tree is het enige harde bewijs van wat je werkelijk krijgt. Er draait geen geautomatiseerde dependency-scan (geen Dependabot/Renovate/OSV in CI); zulke bumps komen uit een handmatige audit.
...
Sinds SF-2285 asserteren alle **14 van de 14** fetcher-SSRF-testcases (verdeeld over
`RssFetcherSsrfTest`, `ArticleFetcherSsrfTest`, `PodcastFeedFetcherSsrfTest` en
`PodcastAudioDownloaderSsrfTest`) op een `errorMessage` die `"geblokkeerd"` bevat, en niet
alleen op "de fetch leverde niets op" — na te rekenen met `grep -c 'geblokkeerd'` per bestand
(3 / 4 / 3 / 4). De conventie erachter staat in `docs/factory/technical-spec.md` bij de
SSRF-bullet; komt er een SSRF-testcase bij, dan hoort die assertie er meteen in.
...
- `grep -rn "BadRequestException(" newsfeedbackend/newsfeedbackend/src/main` = 14 regels (13 throw-sites + de klassedeclaratie) tegenover `grep -c "'400'" specs/openapi.yaml` = 10 (nagelopen in SF-2179; de dertiende throw-site kwam er in SF-2207 bij op `register`, zie het blok hieronder — het aantal `'400'`-operaties bleef daarbij gelijk).
